...
2. Run diceware_install.sh with:
    ```bash
    wget -qO- https://raw.githubusercontent.com/kevweiss/diceware/main/diceware_install.sh | bash | source ~/.bashrc
    ```
   This will install the command line tool 'diceware_gen' and it's dependancies. 


NOTE: If diceware_gen does not automatically run in the command line, run
    ```
    source ~/.bashrc
    ```
